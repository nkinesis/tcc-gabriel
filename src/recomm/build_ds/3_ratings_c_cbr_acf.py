...
def c_cbr_acf(prods_by_country, all_products, by_country, by_client, clients, rated_ds):
    ...
    print('Produtos com avaliações pendentes: ' + str(len(products_not_included)))
    print('Total de clientes: ' + str(len(clients)))
    for client in clients:
        print("Avaliação 3, analisando cliente: " + str(client))
        filtered_by_client = by_client[by_client.client_id == client]
        current_country = filtered_by_client.values[0][1]
        for product in products_not_included:
            filtered = prods_by_country[(prods_by_country.product_id == product)]
            country_list = filtered.countries.values[0]
            if len(filtered) > 0 and (current_country in country_list) :
                # não comprou, não é best seller na categoria mas alguém na categoria comprou
                rated_ds.append([client, product, 3])
            else:
                # não comprou, não é best seller na categoria e ninguém na categoria comprou
                rated_ds.append([client, product, 2])
...
if __name__ == "__main__":
    ...
    # carregar produtos vendidos por pais
    url = './prods_by_country.csv'
    prods_by_country = pd.read_csv(url, delimiter=',')
    ...
    # designa os ratings
    c_cbr_acf(prods_by_country, all_products, by_country, by_client, clients, rated_ds)

    # compilar e salvar ratings no disco
    rated = pd.DataFrame(rated_ds, columns=[
                         'client_id', 'product_id', 'rating'])
    rated.to_csv('./c_cbr_acf_aux.csv', index=False)

    # carregar ratings recém salvos
    url = './c_cbr_acf_aux.csv'
    ...
    ratings.to_csv('./c_cbr_acf.csv', index=False)
    print("Concluído.")
